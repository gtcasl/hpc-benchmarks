...
c---------------------------------------------------------------------
     ...
     >              - tmp1 * dx5
               ...
               lhs(1,1,bb,i,j) = 1.d0
     >              + tmp1 * 2.d0 * njac(1,1,i,j)
     >              + tmp1 * 2.d0 * dx1
               lhs(1,2,bb,i,j) = tmp1 * 2.d0 * njac(1,2,i,j)
               lhs(1,3,bb,i,j) = tmp1 * 2.d0 * njac(1,3,i,j)
               lhs(1,4,bb,i,j) = tmp1 * 2.d0 * njac(1,4,i,j)
               lhs(1,5,bb,i,j) = tmp1 * 2.d0 * njac(1,5,i,j)

               lhs(2,1,bb,i,j) = tmp1 * 2.d0 * njac(2,1,i,j)
               lhs(2,2,bb,i,j) = 1.d0
     >              + tmp1 * 2.d0 * njac(2,2,i,j)
     >              + tmp1 * 2.d0 * dx2
               lhs(2,3,bb,i,j) = tmp1 * 2.d0 * njac(2,3,i,j)
               lhs(2,4,bb,i,j) = tmp1 * 2.d0 * njac(2,4,i,j)
               lhs(2,5,bb,i,j) = tmp1 * 2.d0 * njac(2,5,i,j)

               lhs(3,1,bb,i,j) = tmp1 * 2.d0 * njac(3,1,i,j)
               lhs(3,2,bb,i,j) = tmp1 * 2.d0 * njac(3,2,i,j)
               lhs(3,3,bb,i,j) = 1.d0
     >              + tmp1 * 2.d0 * njac(3,3,i,j)
     >              + tmp1 * 2.d0 * dx3
               lhs(3,4,bb,i,j) = tmp1 * 2.d0 * njac(3,4,i,j)
               lhs(3,5,bb,i,j) = tmp1 * 2.d0 * njac(3,5,i,j)

               lhs(4,1,bb,i,j) = tmp1 * 2.d0 * njac(4,1,i,j)
               lhs(4,2,bb,i,j) = tmp1 * 2.d0 * njac(4,2,i,j)
               lhs(4,3,bb,i,j) = tmp1 * 2.d0 * njac(4,3,i,j)
               lhs(4,4,bb,i,j) = 1.d0
     >              + tmp1 * 2.d0 * njac(4,4,i,j)
     >              + tmp1 * 2.d0 * dx4
               lhs(4,5,bb,i,j) = tmp1 * 2.d0 * njac(4,5,i,j)

               lhs(5,1,bb,i,j) = tmp1 * 2.d0 * njac(5,1,i,j)
               lhs(5,2,bb,i,j) = tmp1 * 2.d0 * njac(5,2,i,j)
               lhs(5,3,bb,i,j) = tmp1 * 2.d0 * njac(5,3,i,j)
               lhs(5,4,bb,i,j) = tmp1 * 2.d0 * njac(5,4,i,j)
               lhs(5,5,bb,i,j) = 1.d0
     >              + tmp1 * 2.d0 * njac(5,5,i,j)
     >              + tmp1 * 2.d0 * dx5
