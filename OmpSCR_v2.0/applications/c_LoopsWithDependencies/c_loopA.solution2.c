...
DESCRIPTION:
		...
		Optimization: Copy only the border element between threads
		Version: Parallel regions containing the outer loops
				2 barriers
COMMENTS:
REFERENCES:
BASIC PRAGMAS:	parallel, barrier
USAGE:		c_loopA.solution2 <size> <numiter>
...
#pragma omp parallel default(none) shared(V,size,nthreads,numiter) private(iter,thread,limitL,limitR,border,i)
	{
	/* 3.1. GET THREAD NUMBER */
	thread = omp_get_thread_num();

	/* 3.2. COMPUTE LIMIT INDEX */
	limitL = thread*size;
...
#pragma omp 	barrier
		...
		/* 3.3.4. COMPUTE LAST ELEMENT (EXCEPT LAST THREAD) */
