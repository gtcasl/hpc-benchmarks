...
!
!			Interleave synchronization and copies on the
!			hand-shake with left and right neighbors
!		
!COMMENTS:
...
!
!USAGE: 	f_cell.solution6 <size_x> <size_y> <numiter>
!
...
!$OMP	private(iter,thread,limitL,limitR,neigL,neigR,i,j)
	...
	do iter=1,numiter
		...
		! 3.3.1. LEFT NEIGHBOR
		! 3.3.1.1. WAIT FOR LEFT NEIGHBOR TO FINISH USING OUR OLD DATA
		if (thread .NE. 0) then
!$OMP			flush(syncL)
...
!$OMP				flush(syncL)
		...
		! 3.3.1.2. COPY/SAVE VALUES NEEDED BY LEFT THREAD
		oldM(1:size_x, limitL) = M(1:size_x, limitL)
		! 3.3.1.3. INFORM LEFT NEIGHBOR ABOUT I HAVE FINISHED THE COPY 
		syncL(thread)=1
!$OMP		flush(syncL)

		! 3.3.2. RIGHT NEIGHBOR
		! 3.3.2.1. WAIT FOR RIGHT NEIGHBOR TO FINISH USING OUR OLD DATA
...
!$OMP				flush(syncR)
		...
		! 3.3.2.2. COPY/SAVE VALUES NEEDED BY RIGHT THREAD
		oldM(1:size_x, limitR) = M(1:size_x, limitR)
		! 3.3.2.3. INFORM RIGHT NEIGHBOR ABOUT I HAVE FINISHED THE COPY 
		syncR(thread)=1
!$OMP		flush(syncR)
