...
!
!USAGE: 	./f_cell.solution8 <size_x> <size_y> <numiter>
!
...
!$OMP	BARRIER
	...
	do iter=1,numiter
		...
		! 3.4.1. LEFT NEIGHBOR
		! 3.4.1.1. WAIT FOR LEFT NEIGHBOUR TO FINISH BEFORE COPY
		if (thread .NE. 0) then
			call OMP_SET_LOCK(readR(neigL))
		end if
		! 3.4.1.2. COPY/SAVE VALUES NEEDED BY LEFT THREAD
		oldM(1:size_x, limitL) = M(1:size_x, limitL)
		! 3.4.1.3. INFORM TO LEFT NEIGHBOR THAT COPY HAVE BEEN DONE
		if (thread .NE. 0) then
			call OMP_UNSET_LOCK(writeR(neigL))
		end if

		! 3.4.2. RIGHT NEIGHBOR
		! 3.4.2.1. WAIT FOR RIGHT NEIGHBOUR TO FINISH BEFORE COPY
		if (thread .NE. nthreads-1) then
			call OMP_SET_LOCK(readL(neigR))
		end if
		! 3.4.2.2. COPY/SAVE VALUES NEEDED BY RIGHT THREAD
		oldM(1:size_x, limitR) = M(1:size_x, limitR)
		! 3.4.2.3. INFORM TO RIGHT NEIGHBOR THAT COPY HAVE BEEN DONE
		if (thread .NE. nthreads-1) then
