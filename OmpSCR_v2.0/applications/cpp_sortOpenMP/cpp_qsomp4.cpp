...
/*******************************************************************************
  ...
  USAGE:             ./cpp_qsomp4.par 1000000 2 1000
...
#				pragma omp critical
		...
		if (i - 1 - q > switchThresh) {
			myBorder = std::make_pair(q, i - 1);
			 ...
			 /* Note that the calls to the member functions of globalTodoStack
			  * are not in any way protected from race conditions! This is only
			  * safe to use, if your STL is threadsafe and therefore does this
			  * for you!
			  */
